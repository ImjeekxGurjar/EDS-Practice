...
function embedYoutube(url, replacePlaceholder, autoplay) {
  ...
  $(document).ready(function(){
    ...
    $('.video-container').slick({
        ...
        slidesToScroll: 1,
        autoplay: true,
        autoplaySpeed: 20000, // Adjust as needed
        arrows: true,
        dots: true,
        infinite: true,
    ...
    // Iterate over each video placeholder

    var totalVideos = $('.video').length;

    $('.video').each(function(index) {
        // Create the navigation container for this video placeholder
        var $navContainer = $('<div class="video-navigation"></div>');
    
        // Create the "Previous" button and append it to the navigation container
        var $prevButton = $('<button class="prev-button">Previous</button>');
        $navContainer.append($prevButton);
    
        // Create the ul element for this video placeholder
        var $ul = $('<ul class="video-options"></ul>');
    
        // Generate options dynamically based on the total number of video placeholders
        var options = [];
        ...
        options.forEach(function(option, optionIndex) {
            var $li = $('<li><a href="#" data-slide-index="' + optionIndex + '">' + option + '</a></li>');
            $ul.append($li);
        });
    
        // Append the ul to the navigation container
        $navContainer.append($ul);
    
        // Create the "Next" button and append it to the navigation container
        var $nextButton = $('<button class="next-button">Next</button>');
        $navContainer.append($nextButton);
    
        // Insert the navigation container after the video placeholder
